...
if($user_id==1 || $user_id ==5){
    ...
    $activenews = App\Models\NewsPost::where('status',1)->where('user_id',$user_id)->get();
}
...
if($user_id==1 || $user_id ==5){
    ...
    $inactivenews = App\Models\NewsPost::where('status',0)->where('user_id',$user_id)->get();
}
if($user_id==1 || $user_id ==5){
    $breakingnews = App\Models\NewsPost::where('breaking_news',1)->get();
}
else{
    $breakingnews = App\Models\NewsPost::where('breaking_news',1)->where('user_id',$user_id)->get();
}
            ...
            <div class="page-title-right">
                <ol class="breadcrumb m-0">
                    <a href="{{ route('add.news.post') }}" class="btn btn-blue waves-effect waves-light">Add News Post</a>
                                    ...
                                    <h4 class="page-title">Add News Post  
                                    <span class="btn btn-danger">{{count($allnews)}}</span></h4>
                                </div>
                            </div>
                        </div>     
                        <!-- end page title --> 
